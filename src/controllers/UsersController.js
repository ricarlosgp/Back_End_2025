...
class UsersController {
    ...
    async update(request, response) {//criando funcionalidade async/assíncrona de atualização do usuário e irei precisar do request e response
        ...
        const { id } = request.params; //pegando o {id} de request.params

        const database = await sqliteConnection(); //criando minha conexão com o banco de dados
        const user = await database.get("SELECT * FROM users WHERE id = (?)", [id]);//buscando em todos * os campos da tabela usuário onde/WHERE o id seja igual(?), aí passo o id do usuário que estou procurando [id]    
        ...
        await database.run(`
            ...
            WHERE id = ?`,
            [user.name, user.email, user.password, id] //esse array deverá estar na mesma sequencia da lista acima do await        
        );
